...
/**
 * The structural shape every lookup master this screen touches shares —
 * `CompanyLookupBase` and `DriveLookupBase` both satisfy it, which is what
 * lets `activeLookups` / `assertSelectable` serve the Drive Attributes
 * masters without duplicating themselves.
 */
type LookupRow = {
  id: number;
  name: string;
...
const yearOption = (y: PassoutYear) => ({
  id: y.id,
...
const sortedChips = (rows: LookupRow[] | undefined) =>
  [...(rows ?? [])]
...
export class CrViewService {
  ...
  async list(employeeId: number, query: CrViewListQueryDto) {
    ...
    return {
      ...
      items: roles.map((r) => ({
        ...
        record: this.mapRecord(records.get(r.id)),
      })),
    };
  }
  ...
  async getRow(employeeId: number, jobRoleId: number, passoutYearId: number) {
    ...
    return {
      ...
      record: this.mapRecord(records.get(role.id)),
    };
  }
  ...
  private async recordsFor(
    ...
    const rows = await this.records.find({
      ...
      // One query per relation instead of one join with all five collections
      // multiplied together — five arrays in a single join is a cartesian
      // product per record.
  ...
  private mapRecord(row: CompanyJobRoleYear | undefined) {
    return row
      ? {
          id: row.id,
          record_code: row.record_code,
          // Sorted here because TypeORM does not order a relation array, and
          // the chips should read in the master's configured order.
          relationship_types: sortedChips(row.relationship_types),
          // `null` means nothing chosen — the screen renders
          // `scope.default_status` in its place. Deliberately NOT resolved to
          // the default here: the two states look different on screen.
          current_status: row.current_status ? chip(row.current_status) : null,
          designations: sortedChips(row.designations),
          // Programmes have no sort_order and show their display name — the
          // same resolution `programmeOptions` uses for the picker.
