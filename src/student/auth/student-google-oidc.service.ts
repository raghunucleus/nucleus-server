...
export class StudentGoogleOidcService {
  ...
  async verifyIdToken(idToken: string): Promise<VerifiedStudentGoogleIdentity> {
    ...
    // `GOOGLE_STUDENT_OIDC_CLIENT_ID=` in .env still falls through to admin.
    const raw =
      nonEmpty(this.config.get<string>('GOOGLE_STUDENT_OIDC_CLIENT_ID')) ??
    ...
    if (allowedDomains.length > 0) {
      ...
      if (!allowedDomains.includes(domain)) {
        this.logger.warn(
          `Rejected student Google sign-in: domain="${domain}" hd="${payload.hd ?? ''}"`,
        );
