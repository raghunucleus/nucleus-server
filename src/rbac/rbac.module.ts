...
@Module({
  imports: [
    TypeOrmModule.forFeature([
      ...
      // approval-action approver membership and attendance-group in-charge
      // membership — see PermissionsService.deriveRequestScreens. All three
      // entities are owned by other modules and repo-injected here rather than
      // imported as modules: ApprovalApproversModule and AdminModule import
      // RbacModule (to bust the cache when a membership changes), so the
      // dependency must only go one way.
