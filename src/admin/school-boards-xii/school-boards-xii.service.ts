...
import type { SchoolBoardsXiiSortField } from '../dto/list-school-boards-xii.dto';
import { SchoolBoardXii } from '../entities/school-board-xii.entity';

export interface ListSchoolBoardsXiiResult {
  rows: SchoolBoardXii[];
  total: number;
  page: number;
  pageSize: number;
...
const SORT_COLUMN: Record<SchoolBoardsXiiSortField, string> = {
  name: 'name',
  code: 'code',
  status: 'is_active',
...
interface CreateSchoolBoardXiiInput {
  name: string;
  code: string;
  description?: string | null;
}

interface UpdateSchoolBoardXiiInput {
  name?: string;
  code?: string;
  description?: string | null;
}
...
@Injectable()
export class SchoolBoardsXiiService {
  constructor(
    @InjectRepository(SchoolBoardXii)
    private readonly boards: Repository<SchoolBoardXii>,
  ) {}
  ...
  async getOne(id: number): Promise<SchoolBoardXii> {
    const row = await this.boards.findOne({ where: { id } });
    if (!row) throw new NotFoundException('XIIth board not found');
    return row;
  }

  async create(input: CreateSchoolBoardXiiInput): Promise<SchoolBoardXii> {
  ...
  async setActive(id: number, active: boolean): Promise<SchoolBoardXii> {
    const row = await this.boards.findOne({ where: { id } });
    if (!row) throw new NotFoundException('XIIth board not found');
