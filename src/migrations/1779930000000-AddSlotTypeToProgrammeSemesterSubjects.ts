...
export class AddSlotTypeToProgrammeSemesterSubjects1779930000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "programme_semester_subjects" ADD "slot_type" character varying(16)`,
    );
    ...
    await queryRunner.query(
      `UPDATE "programme_semester_subjects" SET "slot_type" = 'open_elective' WHERE "subject_id" IS NULL`,
    );

    await queryRunner.query(
      `ALTER TABLE "programme_semester_subjects" ADD CONSTRAINT "CHK_pss_slot_type_with_placeholder" CHECK ((subject_id IS NOT NULL AND slot_type IS NULL) OR (subject_id IS NULL AND slot_type IN ('open_elective', 'honors', 'minors')))`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "programme_semester_subjects" DROP CONSTRAINT "CHK_pss_slot_type_with_placeholder"`,
    );
    await queryRunner.query(
      `ALTER TABLE "programme_semester_subjects" DROP COLUMN "slot_type"`,
    );
  }
}
