...
export class CreateStudentGroups1779840000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE "student_groups" ("id" SERIAL NOT NULL, "student_id" integer NOT NULL, "attendance_group_id" integer, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "UQ_student_groups_student_id" UNIQUE ("student_id"), CONSTRAINT "PK_student_groups_id" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_student_groups_attendance_group_id" ON "student_groups" ("attendance_group_id")`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "student_groups" ADD CONSTRAINT "FK_student_groups_student_id" FOREIGN KEY ("student_id") REFERENCES "students"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "student_groups" ADD CONSTRAINT "FK_student_groups_attendance_group_id" FOREIGN KEY ("attendance_group_id") REFERENCES "attendance_groups"("id") ON DELETE SET NULL ON UPDATE NO ACTION`,
    );
    // Carry over any memberships from the old join table (DISTINCT ON keeps
    // one group per student), then drop it.
    await queryRunner.query(
      `INSERT INTO "student_groups" ("student_id", "attendance_group_id") SELECT DISTINCT ON ("student_id") "student_id", "attendance_group_id" FROM "attendance_group_students" ORDER BY "student_id", "id" DESC`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `INSERT INTO "attendance_group_students" ("attendance_group_id", "student_id") SELECT "attendance_group_id", "student_id" FROM "student_groups" WHERE "attendance_group_id" IS NOT NULL`,
    );
