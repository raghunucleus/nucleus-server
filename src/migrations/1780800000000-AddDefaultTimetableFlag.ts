...
export class AddDefaultTimetableFlag1780800000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "timetables" ADD "is_default" boolean NOT NULL DEFAULT false`,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX "UQ_timetables_default_per_group" ON "timetables" ("programme_semester_id", "attendance_group_id") WHERE "is_default" = TRUE`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP INDEX "public"."UQ_timetables_default_per_group"`,
    );
    await queryRunner.query(
      `ALTER TABLE "timetables" DROP COLUMN "is_default"`,
    );
  }
}
